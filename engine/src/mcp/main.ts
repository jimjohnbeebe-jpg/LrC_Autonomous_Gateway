...
//
// Start-up: take the instance lock and start connecting to Lightroom (bridge-gate.ts), then serve
// MCP. Tools wait up to 5 s for the bridge, so a call made right after start-up does not fail while
// the connection is still being made. Old previews are purged when this engine takes the lock (PRD
// NFR-6), and again at shutdown if it holds it: an engine without the lock never touches the shared
// previews folder, where the engine that holds it may have a preview waiting to be read.
