...
- Anything only Jim can observe is asked by the harness and saved with the results. By default the harness asks in a Lightroom dialog (tick boxes / yes-no). Jim may choose another way for a given check, such as y/n questions in the PowerShell window running it. His choice is recorded, with the date, in that check's report ("Purpose"), and it then counts as following this rule. [stated: Jim, 2026-09-26, "Amend rule 04"; first used for the Phase 1 check, `docs\reports\phase1\PHASE1.md`]
