...
**Also seen:** after run 2 ended, the status file still had `send_connected: true` while `receive_connected` was false [handle: `P1\bridge_status_after_run2.json`, written at 20:30:01Z, 100 s after the last message (`seconds_since_last_message`)]. So the send socket had not noticed its client leaving. The receive socket had: the log has `bridge: receive: closed` at 13:28:21.181 local time, and no send-side line follows [handle: `P1\p1_bridge_log_2026-09-26T20-28-20-636Z.txt`, line 20, the last line]. After run 1 the same log does show `bridge: send: closed`, but at 13:27:07, about two minutes after the engine left (13:25:11) and 25 s before Lightroom started again (13:27:32) [handle: same log, lines 9-12]. That close most likely came from Lightroom shutting down, not from the client leaving [inference]. That matches S2 [handle: `docs\reports\phase0\S2.md` "Consequences": the send socket did not fire `onClosed` when the client disconnected]. The plugin is written to rebind the send socket when the next engine connects on the receive side while the send socket still looks connected [handle: `plugin\LrC-AVG.lrplugin\Bridge.lua`, receive `onConnected`; P-13]. Whether that rebind works in Lightroom is [unverified]: runs 1 and 2 were the first engine connection of each Lightroom session, so no rebind was needed.
